...
function update_phone(data) {
	if (!phones[data.id]) {
		phones[data.id] = new Object();
	}
	phones[data.id]['latitude'] = data.latitude;
	phones[data.id]['longitude'] = data.longitude;
	phones[data.id]['altitude'] = data.altitude;
	if (phones[data.id]['marker'] == null) {
		var image = 'images/smartphone.png';
		phones[data.id]['marker'] = new google.maps.Marker({
			map: map,
			animation: google.maps.Animation.DROP,
			icon: image,
		});
	...
	phones[data.id]['marker'].setPosition(new google.maps.LatLng(data.latitude, data.longitude));
}